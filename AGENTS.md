...
- Перед коммитами выполни **Identity & Signing Bootstrap** из `COMMITS.md` (idempotent: резолвит личность и ключ через `gh`/ssh-agent, ставит local git-config, чинит `core.sshcommand`). Не гадай по ключам вручную — ключ определяется по совпадению с GitHub signing-ключами аккаунта.
