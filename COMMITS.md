...
8. **Коммиты и пуш — только по явному требованию пользователя.** Агент НЕ создаёт коммиты сам по себе (не плодит мусорные/частые коммиты без запроса): сначала сделать работу, предложить снapshot изменений и сформулировать коммит, дождаться явного «коммить». Создавать коммит — только после такого подтверждения. В `origin` пушить — дополнительно после явного словесного подтверждения («пушь», «да»). Bypass в Ruleset даёт право, но не отменяет процессного правила.
...
- Коммит создавать только по явному требованию пользователя (см. Правило 8); не плодить частые/мусорные коммиты без запроса.
