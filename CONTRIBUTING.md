...
Каждый коммит подписывается SSH-ключом. Настройка выполняется локально процедурой **Identity & Signing Bootstrap** из `COMMITS.md` — никаких внешних CLI не нужно. Личность берётся из локального git-config (или задаётся один раз), ключ — из `~/.ssh` (если несколько — выбираешь сам). Нужен SSH-signing-ключ, зарегистрированный на твоём GitHub-аккаунте. По умолчанию используется noreply-адрес; если хочешь реальный email — задай `git config --local user.email` (верифицированный на GitHub) перед bootstrap.
