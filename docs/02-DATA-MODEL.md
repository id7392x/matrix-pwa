...
3. Делаем put в events с PK = eventId (сохраняем txnId для связи)  
...
## Retention событий и ссылочная целостность  
 ...
 Защита** **связей** (Reply/Thread):** Запрещено удалять события, если на них ссылаются существующие в базе свежие события (⁠m.relates_to⁠, ⁠m.in_reply_to⁠, ⁠thread_root⁠). Если родительское событие все же вычищается, перед удалением создает упрощенный контекстный DTO-снимок (stub) для сохранения отображения цитаты в UI.  
