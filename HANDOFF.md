...
- `ff008ea` — **пилюля «верификация + создание чата»** (см. §6): одна пилюля в шапке (38px = «Edit»), verify-кнопка + no-op карандаш «New chat», `session-widget-leave` = фейд+схлопывание; тест на карандаш на верифицированной сессии. Доки `05-UI-E2EE.md` §7.4 поправлены.
...
## 6. Пилюля «верификация + создание чата» — ВЫПОЛНЕНО (2026-08-31, `ff008ea`)

**Проблема (от юзера):** в `683dceb` удалили старую пилюлю [✓-read «Mark all read» + ✏️ «New message»] — вместе с ней исчезла кнопка создания чата. Виджет верификации стал отдельно стоящей таблеткой `size-12` — больше кнопки «Edit» (~38px).

**Итог (согласовано с юзером, реализовано):** кнопка верификации и создания чата — в ОДНОЙ пилюле шапки, размер = «Edit» (38px: p-1 + size-7 + border 2; иконки size-4; верификация слева, карандаш справа; absolute-обёртка убрана, всё в потоке). Карандаш «New chat» — no-op (создание комнат — Слайс 8). `session-widget-leave` — фейд+схлопывание вместо улёта вправо (позиция больше не absolute); после «✓»-вспышки пилюля мгновенно сжимается до карандаша (`ponytail:`). Логика виджета не тронута: `unverified`-derived, `widgetVisible`/`flash`/`hide` = `$state`, `prevUnverified` — plain-переменная, `$effect` + `setTimeout(450)` + `onanimationend`. Обработчики — script-функции. Тест: «New chat» присутствует и на верифицированной сессии (остальные 4 виджет-теста не менялись, aria-label те же). Коммит кода `ff008ea` (автор id7392x + Co-authored-by OpenCode, подпись G), доки — отдельным `docs(ui)`.

### 6.6. Svelte-грабли (дважды кусались)
